...
exports.queryEng = function(req, res) {
  request("http://www.dict.com/Englez-roman/" + encodeURIComponent(req.params.query) + "?", function(error, response, html){
    if(!error){
        ...
        var word, gender, rating;

        var word = String(getFirstWord( $('.entry').find('tr').eq(1).find('span.lex_ful_tran').first().text() )).latinize();


        if ( word ) {
          request({
            method: "GET",
            url: "http://dictionare.com/phpdic/nouns.php?field0=" + word,
            encoding: null,
            ...
            var gender = base.first().find('td').first().text().split('Genre: ')[1];

            var indefinite = base.eq(3);
            var definite = base.eq(4);

            var iWord = indefinite.find('font').eq(4).text().split(" ")[1];
            var iPlural = indefinite.find('font').eq(19).text().split(" ")[1];
